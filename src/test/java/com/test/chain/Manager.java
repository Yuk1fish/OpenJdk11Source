...
public class Manager extends Handler {
    ...
    public boolean process(LeaveRequest leaveRequest) {
        ...
        System.out.println(String.format(log, this.name, leaveRequest.getName(), leaveRequest.getLeaveDays(), result == true ? "批准" : "不批准"));

        if (!result) {  // 不批准
            return false;
        } else if (leaveRequest.getLeaveDays() < 7) { // 批准且天数小于7
            return true;
        }
